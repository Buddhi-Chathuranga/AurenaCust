...
TYPE Indicator_Rec IS RECORD
  (company_id                     BOOLEAN := FALSE,
   ...
   name                           BOOLEAN := FALSE,
   picking_lead_time              BOOLEAN := FALSE);
...
IS
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(company_id_, branch_id_, inventory_id_),
                            Formatted_Key___(company_id_, branch_id_, inventory_id_));
   Error_SYS.Fnd_Too_Many_Rows(Trn_Inventory_API.lu_name_, NULL, methodname_);
...
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(company_id_, branch_id_, inventory_id_),
                            Formatted_Key___(company_id_, branch_id_, inventory_id_));
   Error_SYS.Fnd_Record_Not_Exist(Trn_Inventory_API.lu_name_);
...
PROCEDURE Raise_Record_Exist___ (
   rec_ IN trn_inventory_tab%ROWTYPE )
IS
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(rec_.company_id, rec_.branch_id, rec_.inventory_id),
                            Formatted_Key___(rec_.company_id, rec_.branch_id, rec_.inventory_id));
   Error_SYS.Fnd_Record_Exist(Trn_Inventory_API.lu_name_);
...
IS
BEGIN
   Error_SYS.Fnd_Item_Format(Trn_Inventory_API.lu_name_, name_, value_);
...
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(rec_.company_id, rec_.branch_id, rec_.inventory_id),
                            Formatted_Key___(rec_.company_id, rec_.branch_id, rec_.inventory_id));
   Error_SYS.Fnd_Record_Modified(Trn_Inventory_API.lu_name_);
...
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(company_id_, branch_id_, inventory_id_),
                            Formatted_Key___(company_id_, branch_id_, inventory_id_));
   Error_SYS.Fnd_Record_Locked(Trn_Inventory_API.lu_name_);
...
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(company_id_, branch_id_, inventory_id_),
                            Formatted_Key___(company_id_, branch_id_, inventory_id_));
   Error_SYS.Fnd_Record_Removed(Trn_Inventory_API.lu_name_);
...
FUNCTION Lock_By_Id___ (
   objid_      IN VARCHAR2,
   objversion_ IN VARCHAR2 ) RETURN trn_inventory_tab%ROWTYPE
IS
   row_locked  EXCEPTION;
   PRAGMA      EXCEPTION_INIT(row_locked, -0054);
   rec_        trn_inventory_tab%ROWTYPE;
BEGIN
   SELECT *
      INTO  rec_
...
FUNCTION Lock_By_Keys___ (
   company_id_ IN VARCHAR2,
   branch_id_ IN VARCHAR2,
   inventory_id_ IN NUMBER) RETURN trn_inventory_tab%ROWTYPE
IS
   rec_        trn_inventory_tab%ROWTYPE;
BEGIN
   BEGIN
   ...
   EXCEPTION
      WHEN no_data_found THEN
         Raise_Record_Removed___(company_id_, branch_id_, inventory_id_);
      WHEN too_many_rows THEN
         Raise_Too_Many_Rows___(company_id_, branch_id_, inventory_id_, 'Lock_By_Keys___');
   END;
END Lock_By_Keys___;
...
BEGIN
   ...
   EXCEPTION
      WHEN row_locked THEN
         Raise_Record_Locked___(company_id_, branch_id_, inventory_id_);
      WHEN too_many_rows THEN
         Raise_Too_Many_Rows___(company_id_, branch_id_, inventory_id_, 'Lock_By_Keys___');
      WHEN no_data_found THEN
         Raise_Record_Removed___(company_id_, branch_id_, inventory_id_);
...
FUNCTION Get_Object_By_Id___ (
   objid_ IN VARCHAR2 ) RETURN trn_inventory_tab%ROWTYPE
IS
   lu_rec_ trn_inventory_tab%ROWTYPE;
BEGIN
   SELECT *
      INTO  lu_rec_
      FROM  trn_inventory_tab
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN lu_rec_;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(company_id_, branch_id_, inventory_id_, 'Get_Object_By_Keys___');
...
IS
   dummy_ NUMBER;
BEGIN
   IF (company_id_ IS NULL OR branch_id_ IS NULL OR inventory_id_ IS NULL) THEN
      RETURN FALSE;
   END IF;
   SELECT 1
      INTO  dummy_
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN FALSE;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(company_id_, branch_id_, inventory_id_, 'Check_Exist___');
END Check_Exist___;
...
EXCEPTION
   WHEN no_data_found THEN
      ...
      objversion_ := NULL;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(company_id_, branch_id_, inventory_id_, 'Get_Id_Version_By_Keys___');
...
BEGIN
   ...
   WHILE (Client_SYS.Get_Next_From_Attr(attr_, ptr_, name_, value_)) LOOP
      ...
      WHEN ('NAME') THEN
         newrec_.name := value_;
         indrec_.name := TRUE;
      WHEN ('PICKING_LEAD_TIME') THEN
         newrec_.picking_lead_time := Client_SYS.Attr_Value_To_Number(value_);
         indrec_.picking_lead_time := TRUE;
      ELSE
...
FUNCTION Pack___ (
   rec_ IN trn_inventory_tab%ROWTYPE ) RETURN VARCHAR2
IS
...
BEGIN
   ...
   END IF;
   IF (rec_.picking_lead_time IS NOT NULL) THEN
      Client_SYS.Add_To_Attr('PICKING_LEAD_TIME', rec_.picking_lead_time, attr_);
   END IF;
   RETURN attr_;
END Pack___;
...
BEGIN
   ...
   IF (indrec_.name) THEN
      Client_SYS.Add_To_Attr('NAME', rec_.name, attr_);
   END IF;
   IF (indrec_.picking_lead_time) THEN
      Client_SYS.Add_To_Attr('PICKING_LEAD_TIME', rec_.picking_lead_time, attr_);
   END IF;
   RETURN attr_;
END Pack___;
...
FUNCTION Public_To_Table___ (
   public_ IN Public_Rec ) RETURN trn_inventory_tab%ROWTYPE
IS
   rec_ trn_inventory_tab%ROWTYPE;
BEGIN
...
FUNCTION Table_To_Public___ (
   rec_ IN trn_inventory_tab%ROWTYPE ) RETURN Public_Rec
IS
   public_ Public_Rec;
BEGIN
...
FUNCTION Get_Indicator_Rec___ (
   rec_ IN trn_inventory_tab%ROWTYPE ) RETURN Indicator_Rec
IS
...
BEGIN
   ...
   indrec_.picking_lead_time := rec_.picking_lead_time IS NOT NULL;
   RETURN indrec_;
...
FUNCTION Get_Indicator_Rec___ (
   oldrec_ IN trn_inventory_tab%ROWTYPE,
   newrec_ IN trn_inventory_tab%ROWTYPE ) RETURN Indicator_Rec
IS
   indrec_ Indicator_Rec;
BEGIN
   ...
   indrec_.picking_lead_time := Validate_SYS.Is_Changed(oldrec_.picking_lead_time, newrec_.picking_lead_time);
   RETURN indrec_;
END Get_Indicator_Rec___;
...
BEGIN
   ...
   Error_SYS.Check_Not_Null(lu_name_, 'PICKING_LEAD_TIME', newrec_.picking_lead_time);
END Check_Common___;
...
PROCEDURE Check_Insert___ (
   ...
   attr_   IN OUT VARCHAR2 )
IS
   oldrec_ trn_inventory_tab%ROWTYPE;
BEGIN
...
EXCEPTION
   WHEN dup_val_on_index THEN
      ...
      BEGIN
         IF (constraint_ = 'TRN_INVENTORY_RK') THEN
            Error_SYS.Fnd_Rowkey_Exist(lu_name_, newrec_.rowkey);
         ELSIF (constraint_ = 'TRN_INVENTORY_PK') THEN
...
EXCEPTION
   WHEN dup_val_on_index THEN
      ...
      BEGIN
         IF (constraint_ = 'TRN_INVENTORY_RK') THEN
            Error_SYS.Fnd_Rowkey_Exist(Trn_Inventory_API.lu_name_, newrec_.rowkey);
         ELSIF (constraint_ = 'TRN_INVENTORY_PK') THEN
            Raise_Record_Exist___(newrec_);
         ELSE
            Raise_Constraint_Violated___(newrec_, constraint_);
...
BEGIN
   IF (lock_mode_wait_) THEN
      oldrec_ := Lock_By_Keys___(newrec_.company_id, newrec_.branch_id, newrec_.inventory_id);
   ELSE
      oldrec_ := Lock_By_Keys_Nowait___(newrec_.company_id, newrec_.branch_id, newrec_.inventory_id);
   END IF;
...
PROCEDURE Check_Delete___ (
   remrec_ IN trn_inventory_tab%ROWTYPE )
IS
   key_ VARCHAR2(2000);
BEGIN
   key_ := remrec_.company_id||'^'||remrec_.branch_id||'^'||remrec_.inventory_id||'^';
...
PROCEDURE Remove___ (
   remrec_         IN OUT trn_inventory_tab%ROWTYPE,
   lock_mode_wait_ IN     BOOLEAN DEFAULT TRUE )
IS
   oldrec_     trn_inventory_tab%ROWTYPE;
BEGIN
   IF (lock_mode_wait_) THEN
      oldrec_ := Lock_By_Keys___(remrec_.company_id, remrec_.branch_id, remrec_.inventory_id);
   ELSE
      oldrec_ := Lock_By_Keys_Nowait___(remrec_.company_id, remrec_.branch_id, remrec_.inventory_id);
   END IF;
...
IS
   dummy_ trn_inventory_tab%ROWTYPE;
BEGIN
...
PROCEDURE New__ (
   ...
   action_     IN     VARCHAR2 )
IS
   newrec_   trn_inventory_tab%ROWTYPE;
   indrec_   Indicator_Rec;
BEGIN
...
PROCEDURE Remove__ (
   ...
   action_     IN  VARCHAR2 )
IS
   remrec_ trn_inventory_tab%ROWTYPE;
BEGIN
   IF (action_ = 'CHECK') THEN
...
BEGIN
   IF (rowkey_ IS NULL) THEN
      RETURN NULL;
   END IF;
   SELECT company_id, branch_id, inventory_id
      INTO  rec_.company_id, rec_.branch_id, rec_.inventory_id
      FROM  trn_inventory_tab
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN rec_;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(rec_.company_id, rec_.branch_id, rec_.inventory_id, 'Get_Key_By_Rowkey');
...
IS
BEGIN
   IF (NOT Check_Exist___(company_id_, branch_id_, inventory_id_)) THEN
      Raise_Record_Not_Exist___(company_id_, branch_id_, inventory_id_);
   END IF;
END Exist;
...
IS
BEGIN
   RETURN Check_Exist___(company_id_, branch_id_, inventory_id_);
END Exists;
...
PROCEDURE Rowkey_Exist (
   rowkey_ IN VARCHAR2 )
IS
   company_id_ trn_inventory_tab.company_id%TYPE;
   branch_id_ trn_inventory_tab.branch_id%TYPE;
   inventory_id_ trn_inventory_tab.inventory_id%TYPE;
BEGIN
   IF (rowkey_ IS NULL) THEN
      RAISE no_data_found;
   END IF;
   SELECT company_id, branch_id, inventory_id
   INTO  company_id_, branch_id_, inventory_id_
   FROM  trn_inventory_tab
   WHERE rowkey = rowkey_;
EXCEPTION
   WHEN no_data_found THEN
      Raise_Record_Not_Exist___(company_id_, branch_id_, inventory_id_);
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(company_id_, branch_id_, inventory_id_, 'Rowkey_Exist___');
END Rowkey_Exist;
...
FUNCTION Get_Name (
   ...
   inventory_id_ IN NUMBER ) RETURN VARCHAR2
IS
   temp_ trn_inventory_tab.name%TYPE;
BEGIN
   IF (company_id_ IS NULL OR branch_id_ IS NULL OR inventory_id_ IS NULL) THEN
      RETURN NULL;
   END IF;
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN NULL;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(company_id_, branch_id_, inventory_id_, 'Get_Name');
END Get_Name;
...
IS
   rowrec_ trn_inventory_tab%ROWTYPE;
BEGIN
   rowrec_ := Get_Key_By_Rowkey(rowkey_);
   RETURN Get(rowrec_.company_id, rowrec_.branch_id, rowrec_.inventory_id);
END Get_By_Rowkey;
...
BEGIN
   IF (company_id_ IS NULL OR branch_id_ IS NULL OR inventory_id_ IS NULL) THEN
      RETURN NULL;
   END IF;
   SELECT company_id, branch_id, inventory_id, rowid, rowversion, rowkey,
          name
      INTO  temp_
      FROM  trn_inventory_tab
      WHERE company_id = company_id_
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN NULL;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(company_id_, branch_id_, inventory_id_, 'Get');
END Get;
...
FUNCTION Get_Objkey (
   ...
   inventory_id_ IN NUMBER ) RETURN VARCHAR2
IS
   rowkey_ trn_inventory_tab.rowkey%TYPE;
BEGIN
   IF (company_id_ IS NULL OR branch_id_ IS NULL OR inventory_id_ IS NULL) THEN
      RETURN NULL;
   END IF;
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN NULL;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(company_id_, branch_id_, inventory_id_, 'Get_Objkey');
END Get_Objkey;
