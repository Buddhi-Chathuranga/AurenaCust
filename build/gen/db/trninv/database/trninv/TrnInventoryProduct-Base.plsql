...
--  (company_id                     TRN_INVENTORY_PRODUCT_TAB.company_id%TYPE,
--   branch_id                      TRN_INVENTORY_PRODUCT_TAB.branch_id%TYPE,
--   inventory_id                   TRN_INVENTORY_PRODUCT_TAB.inventory_id%TYPE,
--   product_id                     TRN_INVENTORY_PRODUCT_TAB.product_id%TYPE);
...
TYPE Public_Rec IS RECORD
  (company_id                     TRN_INVENTORY_PRODUCT_TAB.company_id%TYPE,
   ...
   product_id                     TRN_INVENTORY_PRODUCT_TAB.product_id%TYPE,
   "rowid"                        rowid,
   rowversion                     TRN_INVENTORY_PRODUCT_TAB.rowversion%TYPE,
   rowkey                         TRN_INVENTORY_PRODUCT_TAB.rowkey%TYPE,
   quantity                       TRN_INVENTORY_PRODUCT_TAB.quantity%TYPE);
...
TYPE Indicator_Rec IS RECORD
  (company_id                     BOOLEAN := FALSE,
   ...
   reorder_level                  BOOLEAN := FALSE,
   reserved_quantity              BOOLEAN := FALSE);
...
IS
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(company_id_, branch_id_, inventory_id_, product_id_),
                            Formatted_Key___(company_id_, branch_id_, inventory_id_, product_id_));
   Error_SYS.Fnd_Too_Many_Rows(Trn_Inventory_Product_API.lu_name_, NULL, methodname_);
...
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(company_id_, branch_id_, inventory_id_, product_id_),
                            Formatted_Key___(company_id_, branch_id_, inventory_id_, product_id_));
   Error_SYS.Fnd_Record_Not_Exist(Trn_Inventory_Product_API.lu_name_);
...
PROCEDURE Raise_Record_Exist___ (
   rec_ IN trn_inventory_product_tab%ROWTYPE )
IS
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(rec_.company_id, rec_.branch_id, rec_.inventory_id, rec_.product_id),
                            Formatted_Key___(rec_.company_id, rec_.branch_id, rec_.inventory_id, rec_.product_id));
   Error_SYS.Fnd_Record_Exist(Trn_Inventory_Product_API.lu_name_);
...
IS
BEGIN
   Error_SYS.Fnd_Item_Format(Trn_Inventory_Product_API.lu_name_, name_, value_);
...
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(rec_.company_id, rec_.branch_id, rec_.inventory_id, rec_.product_id),
                            Formatted_Key___(rec_.company_id, rec_.branch_id, rec_.inventory_id, rec_.product_id));
   Error_SYS.Fnd_Record_Modified(Trn_Inventory_Product_API.lu_name_);
...
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(company_id_, branch_id_, inventory_id_, product_id_),
                            Formatted_Key___(company_id_, branch_id_, inventory_id_, product_id_));
   Error_SYS.Fnd_Record_Locked(Trn_Inventory_Product_API.lu_name_);
...
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(company_id_, branch_id_, inventory_id_, product_id_),
                            Formatted_Key___(company_id_, branch_id_, inventory_id_, product_id_));
   Error_SYS.Fnd_Record_Removed(Trn_Inventory_Product_API.lu_name_);
...
FUNCTION Lock_By_Id___ (
   objid_      IN VARCHAR2,
   objversion_ IN VARCHAR2 ) RETURN trn_inventory_product_tab%ROWTYPE
IS
...
FUNCTION Lock_By_Keys___ (
   ...
   branch_id_ IN VARCHAR2,
   inventory_id_ IN NUMBER,
   product_id_ IN NUMBER) RETURN trn_inventory_product_tab%ROWTYPE
IS
   rec_        trn_inventory_product_tab%ROWTYPE;
BEGIN
   BEGIN
   ...
   EXCEPTION
      WHEN no_data_found THEN
         Raise_Record_Removed___(company_id_, branch_id_, inventory_id_, product_id_);
      WHEN too_many_rows THEN
         Raise_Too_Many_Rows___(company_id_, branch_id_, inventory_id_, product_id_, 'Lock_By_Keys___');
   END;
END Lock_By_Keys___;
...
BEGIN
   ...
   EXCEPTION
      WHEN row_locked THEN
         Raise_Record_Locked___(company_id_, branch_id_, inventory_id_, product_id_);
      WHEN too_many_rows THEN
         Raise_Too_Many_Rows___(company_id_, branch_id_, inventory_id_, product_id_, 'Lock_By_Keys___');
      WHEN no_data_found THEN
         Raise_Record_Removed___(company_id_, branch_id_, inventory_id_, product_id_);
...
FUNCTION Get_Object_By_Id___ (
   objid_ IN VARCHAR2 ) RETURN trn_inventory_product_tab%ROWTYPE
IS
   lu_rec_ trn_inventory_product_tab%ROWTYPE;
BEGIN
   SELECT *
      INTO  lu_rec_
      FROM  trn_inventory_product_tab
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN lu_rec_;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(company_id_, branch_id_, inventory_id_, product_id_, 'Get_Object_By_Keys___');
...
IS
   dummy_ NUMBER;
BEGIN
   IF (company_id_ IS NULL OR branch_id_ IS NULL OR inventory_id_ IS NULL OR product_id_ IS NULL) THEN
      RETURN FALSE;
   END IF;
   SELECT 1
      INTO  dummy_
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN FALSE;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(company_id_, branch_id_, inventory_id_, product_id_, 'Check_Exist___');
END Check_Exist___;
...
EXCEPTION
   WHEN no_data_found THEN
      ...
      objversion_ := NULL;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(company_id_, branch_id_, inventory_id_, product_id_, 'Get_Id_Version_By_Keys___');
...
BEGIN
   ...
   WHILE (Client_SYS.Get_Next_From_Attr(attr_, ptr_, name_, value_)) LOOP
      ...
      WHEN ('REORDER_LEVEL') THEN
         newrec_.reorder_level := Client_SYS.Attr_Value_To_Number(value_);
         indrec_.reorder_level := TRUE;
      WHEN ('RESERVED_QUANTITY') THEN
         newrec_.reserved_quantity := Client_SYS.Attr_Value_To_Number(value_);
         indrec_.reserved_quantity := TRUE;
      ELSE
...
FUNCTION Pack___ (
   rec_ IN trn_inventory_product_tab%ROWTYPE ) RETURN VARCHAR2
IS
...
BEGIN
   ...
   IF (rec_.reorder_level IS NOT NULL) THEN
      Client_SYS.Add_To_Attr('REORDER_LEVEL', rec_.reorder_level, attr_);
   END IF;
   IF (rec_.reserved_quantity IS NOT NULL) THEN
      Client_SYS.Add_To_Attr('RESERVED_QUANTITY', rec_.reserved_quantity, attr_);
   END IF;
   RETURN attr_;
END Pack___;
...
BEGIN
   ...
   IF (indrec_.quantity) THEN
      Client_SYS.Add_To_Attr('QUANTITY', rec_.quantity, attr_);
   END IF;
   IF (indrec_.reorder_level) THEN
      Client_SYS.Add_To_Attr('REORDER_LEVEL', rec_.reorder_level, attr_);
   END IF;
   IF (indrec_.reserved_quantity) THEN
      Client_SYS.Add_To_Attr('RESERVED_QUANTITY', rec_.reserved_quantity, attr_);
   END IF;
   RETURN attr_;
END Pack___;
...
FUNCTION Public_To_Table___ (
   public_ IN Public_Rec ) RETURN trn_inventory_product_tab%ROWTYPE
IS
   rec_ trn_inventory_product_tab%ROWTYPE;
BEGIN
...
FUNCTION Table_To_Public___ (
   rec_ IN trn_inventory_product_tab%ROWTYPE ) RETURN Public_Rec
IS
   public_ Public_Rec;
BEGIN
...
FUNCTION Get_Indicator_Rec___ (
   rec_ IN trn_inventory_product_tab%ROWTYPE ) RETURN Indicator_Rec
IS
...
BEGIN
   ...
   indrec_.reorder_level := rec_.reorder_level IS NOT NULL;
   indrec_.reserved_quantity := rec_.reserved_quantity IS NOT NULL;
   RETURN indrec_;
...
FUNCTION Get_Indicator_Rec___ (
   oldrec_ IN trn_inventory_product_tab%ROWTYPE,
   newrec_ IN trn_inventory_product_tab%ROWTYPE ) RETURN Indicator_Rec
IS
   indrec_ Indicator_Rec;
BEGIN
   ...
   indrec_.reorder_level := Validate_SYS.Is_Changed(oldrec_.reorder_level, newrec_.reorder_level);
   indrec_.reserved_quantity := Validate_SYS.Is_Changed(oldrec_.reserved_quantity, newrec_.reserved_quantity);
   RETURN indrec_;
END Get_Indicator_Rec___;
...
BEGIN
   ...
   Error_SYS.Check_Not_Null(lu_name_, 'REORDER_LEVEL', newrec_.reorder_level);
   Error_SYS.Check_Not_Null(lu_name_, 'RESERVED_QUANTITY', newrec_.reserved_quantity);
END Check_Common___;
...
PROCEDURE Check_Insert___ (
   ...
   attr_   IN OUT VARCHAR2 )
IS
   oldrec_ trn_inventory_product_tab%ROWTYPE;
BEGIN
...
BEGIN
   ...
   INSERT
      INTO trn_inventory_product_tab
      VALUES newrec_
...
EXCEPTION
   WHEN dup_val_on_index THEN
      ...
      BEGIN
         IF (constraint_ = 'TRN_INVENTORY_PRODUCT_RK') THEN
            Error_SYS.Fnd_Rowkey_Exist(lu_name_, newrec_.rowkey);
         ELSIF (constraint_ = 'TRN_INVENTORY_PRODUCT_PK') THEN
...
BEGIN
   newrec_.rowversion := sysdate;
   IF by_keys_ THEN
      UPDATE trn_inventory_product_tab
         SET ROW = newrec_
...
EXCEPTION
   WHEN dup_val_on_index THEN
      ...
      BEGIN
         IF (constraint_ = 'TRN_INVENTORY_PRODUCT_RK') THEN
            Error_SYS.Fnd_Rowkey_Exist(Trn_Inventory_Product_API.lu_name_, newrec_.rowkey);
         ELSIF (constraint_ = 'TRN_INVENTORY_PRODUCT_PK') THEN
            Raise_Record_Exist___(newrec_);
         ELSE
            Raise_Constraint_Violated___(newrec_, constraint_);
...
BEGIN
   IF (lock_mode_wait_) THEN
      oldrec_ := Lock_By_Keys___(newrec_.company_id, newrec_.branch_id, newrec_.inventory_id, newrec_.product_id);
   ELSE
      oldrec_ := Lock_By_Keys_Nowait___(newrec_.company_id, newrec_.branch_id, newrec_.inventory_id, newrec_.product_id);
   END IF;
...
PROCEDURE Check_Delete___ (
   remrec_ IN trn_inventory_product_tab%ROWTYPE )
IS
   key_ VARCHAR2(2000);
BEGIN
   key_ := remrec_.company_id||'^'||remrec_.branch_id||'^'||remrec_.inventory_id||'^'||remrec_.product_id||'^';
...
PROCEDURE Remove___ (
   remrec_         IN OUT trn_inventory_product_tab%ROWTYPE,
   lock_mode_wait_ IN     BOOLEAN DEFAULT TRUE )
IS
   oldrec_     trn_inventory_product_tab%ROWTYPE;
BEGIN
   IF (lock_mode_wait_) THEN
      oldrec_ := Lock_By_Keys___(remrec_.company_id, remrec_.branch_id, remrec_.inventory_id, remrec_.product_id);
   ELSE
      oldrec_ := Lock_By_Keys_Nowait___(remrec_.company_id, remrec_.branch_id, remrec_.inventory_id, remrec_.product_id);
   END IF;
...
IS
   dummy_ trn_inventory_product_tab%ROWTYPE;
BEGIN
...
PROCEDURE New__ (
   ...
   action_     IN     VARCHAR2 )
IS
   newrec_   trn_inventory_product_tab%ROWTYPE;
   indrec_   Indicator_Rec;
BEGIN
...
PROCEDURE Remove__ (
   ...
   action_     IN  VARCHAR2 )
IS
   remrec_ trn_inventory_product_tab%ROWTYPE;
BEGIN
   IF (action_ = 'CHECK') THEN
...
FUNCTION Get_Key_By_Rowkey (
   rowkey_ IN VARCHAR2 ) RETURN trn_inventory_product_tab%ROWTYPE
IS
   rec_ trn_inventory_product_tab%ROWTYPE;
BEGIN
   ...
   SELECT company_id, branch_id, inventory_id, product_id
      INTO  rec_.company_id, rec_.branch_id, rec_.inventory_id, rec_.product_id
      FROM  trn_inventory_product_tab
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN rec_;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(rec_.company_id, rec_.branch_id, rec_.inventory_id, rec_.product_id, 'Get_Key_By_Rowkey');
...
IS
BEGIN
   IF (NOT Check_Exist___(company_id_, branch_id_, inventory_id_, product_id_)) THEN
      Raise_Record_Not_Exist___(company_id_, branch_id_, inventory_id_, product_id_);
   END IF;
END Exist;
...
IS
BEGIN
   RETURN Check_Exist___(company_id_, branch_id_, inventory_id_, product_id_);
END Exists;
...
PROCEDURE Rowkey_Exist (
   rowkey_ IN VARCHAR2 )
IS
   company_id_ trn_inventory_product_tab.company_id%TYPE;
   branch_id_ trn_inventory_product_tab.branch_id%TYPE;
   inventory_id_ trn_inventory_product_tab.inventory_id%TYPE;
   product_id_ trn_inventory_product_tab.product_id%TYPE;
BEGIN
   IF (rowkey_ IS NULL) THEN
      RAISE no_data_found;
   END IF;
   SELECT company_id, branch_id, inventory_id, product_id
   INTO  company_id_, branch_id_, inventory_id_, product_id_
   FROM  trn_inventory_product_tab
   WHERE rowkey = rowkey_;
EXCEPTION
   WHEN no_data_found THEN
      Raise_Record_Not_Exist___(company_id_, branch_id_, inventory_id_, product_id_);
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(company_id_, branch_id_, inventory_id_, product_id_, 'Rowkey_Exist___');
END Rowkey_Exist;
...
IS
   temp_ trn_inventory_product_tab.quantity%TYPE;
BEGIN
   IF (company_id_ IS NULL OR branch_id_ IS NULL OR inventory_id_ IS NULL OR product_id_ IS NULL) THEN
      RETURN NULL;
   END IF;
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN NULL;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(company_id_, branch_id_, inventory_id_, product_id_, 'Get_Quantity');
END Get_Quantity;
...
IS
   rowrec_ trn_inventory_product_tab%ROWTYPE;
BEGIN
   rowrec_ := Get_Key_By_Rowkey(rowkey_);
   RETURN Get(rowrec_.company_id, rowrec_.branch_id, rowrec_.inventory_id, rowrec_.product_id);
END Get_By_Rowkey;
...
BEGIN
   IF (company_id_ IS NULL OR branch_id_ IS NULL OR inventory_id_ IS NULL OR product_id_ IS NULL) THEN
      RETURN NULL;
   END IF;
   SELECT company_id, branch_id, inventory_id, product_id, rowid, rowversion, rowkey,
          quantity
      INTO  temp_
      FROM  trn_inventory_product_tab
      WHERE company_id = company_id_
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN NULL;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(company_id_, branch_id_, inventory_id_, product_id_, 'Get');
END Get;
...
FUNCTION Get_Objkey (
   ...
   product_id_ IN NUMBER ) RETURN VARCHAR2
IS
   rowkey_ trn_inventory_product_tab.rowkey%TYPE;
BEGIN
   IF (company_id_ IS NULL OR branch_id_ IS NULL OR inventory_id_ IS NULL OR product_id_ IS NULL) THEN
      RETURN NULL;
   END IF;
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN NULL;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(company_id_, branch_id_, inventory_id_, product_id_, 'Get_Objkey');
END Get_Objkey;
